...
public class SoundPropagation : MonoBehaviour
{
    ...
    void Start()
    {
        paintAllTiles(floorTilemap, Color.black);
        paintAllTiles(wallTilemap, Color.black);
    ...
    void paintAllTiles(Tilemap tilemap, Color color){
        foreach (Vector3Int tilePosition in tilemap.cellBounds.allPositionsWithin)
        {
            SetTileColour(color, tilePosition, tilemap);
        }
    }
   ...
   void PaintTiles(Vector3Int source)
    {
        ...
        while (queue.Count > 0)
        {
            ...
            Color tileColor = Color.Lerp(Color.black, Color.white, currentLevel);
            ...
            foreach (Vector3Int direction in directions)
            {
                ...
                if (IsWall(neighbor))
                {
                    List<Vector3Int> neighbors = GetNeighbors(neighbor);
                    foreach (Vector3Int neighborNeighbor in neighbors)
                    {
                        if (IsFloor(neighborNeighbor))
                        {
                            Color color = new Color(0.3f, 0.3f, 0.3f); // Darker gray color
                            SetTileColour(color, neighbor, wallTilemap);
                            break;
                        }
                    }
    ...
    void ClearPreviousSoundPropagation()
    {
        ...
        {
            SetTileColour(Color.black, tilePosition, floorTilemap);
        }
